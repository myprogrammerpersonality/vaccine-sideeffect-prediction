...
logging.basicConfig(filename='T2.log', format="%(asctime)s - %(levelname)s - %(message)s", level=logging.INFO)
...
for model in ['Logistic_Regression', 'SVM', 'XGBClassifier', 'RF', 'KNN', 'MLP']:
    temp = pd.read_csv(f'{output_path}/results/D1_{model}_{vaccine_type}_CVTest_{time_to_put}.csv')
    for sideeffect in dose_1_columns:
...
plt.title(f'First Dose of {name_dict[vaccine_type]} Vaccine')
...
for i, model in enumerate(['Logistic_Regression', 'SVM', 'XGBClassifier', 'RF', 'KNN', 'MLP']):
    for j, side in enumerate(dose_1_columns):
...
plt.xticks(rotation=45)
plt.title(f"Dose 1 of {name_dict[vaccine_type]}\nTraining Set")
...
all_results_df.to_csv(f'D1_{vaccine_type}_Best_Param_{time_to_put}.tsv', index=False, sep='\t')
...
result_df = pd.read_csv(f'{output_path}/results/D1_Logistic_Regression_{vaccine_type}_CVTest_{time_to_put}.csv') 

coeff_df = pd.DataFrame(columns=dose_1_columns, index=data.drop(dose_1_columns_full, axis=1).columns, data=np.zeros((len(data.drop(dose_1_columns_full, axis=1).columns), len(dose_1_columns))))
...
coeff_df['Avg'] = [coeff_df.loc[i,:].map(lambda x: abs(x)).mean() for i in coeff_df.index]

coeff_df.to_csv(f'{output_path}/reports/D1_{vaccine_type}_feature_importance_{time_to_put}.csv')
...
sns.scatterplot(x="X", y="Y", size = "S", hue="Sign", data=dfu, sizes=(20, 2000), legend=False)
plt.xticks(rotation=45)
