...
logging.basicConfig(filename='T6.log', format="%(asctime)s - %(levelname)s - %(message)s", level=logging.INFO)
...
for model in ['Logistic_Regression', 'SVM', 'XGBClassifier', 'RF', 'KNN', 'MLP']:
    temp = pd.read_csv(f'{output_path}/results/D2-1_{model}_{vaccine_type}_CVTest_{time_to_put}.csv')
...
plt.title(f'Second Dose of {name_dict[vaccine_type]} Vaccine without Dose 1 info')
...
plt.xticks(rotation=45)
plt.title(f"Dose 2 of {name_dict[vaccine_type]} without Dose 1 info\nTraining Set")
...
all_results_df.to_csv(f'D2-1_{vaccine_type}_Best_Param_{time_to_put}.tsv', index=False, sep='\t')
...
result_df = pd.read_csv(f'{output_path}/results/D2-1_Logistic_Regression_{vaccine_type}_CVTest_{time_to_put}.csv') 
